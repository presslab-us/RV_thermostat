const char* VARIABLES_DEF_YAML PROGMEM = R"~(
    Thermostat:
      ...
      - therm_ac_fail_temp_time:
          ...
          range: 60, 7200, 1
          default: 1800
      - therm_ac_fail_comm_time:
          label: AC fail for comms time (seconds)
          range: 5, 1200, 1
          default: 120
      - therm_ac_recover_time:
          label: AC recover time (seconds)
          range: 0, 1200, 1
          default: 60
      - therm_furn_deadband:
          label: Furnace deadband *C
          default: 1.5
          attribs: min="0.1" max="10" step="0.1"
